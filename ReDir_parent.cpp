...
private:
    ...
    //HANDLE hGlobalStopEvent; //just use g_hGlobalStopEvent
...
public:
    ThreadWrapper() 
    {
        ...
        // WaitDorMultipleObjects(hThread, hThreadStartedEvent, 10sec);
        // if timeout TerminateThread
    }
    static bool CreateThread() 
    {
        auto p = new ThreadWrapper();
        if (p->bStatus) return p;
        delete p;
        return nullptr;
    }
...
protected:
    virtual bool ThreadInit(); // abstract
    virtual DWORD ThreadWorker(); // abstract
    // consider to rework static polymorphism (aka templates)
};
...
int redir_parent(int argc, TCHAR* argv[])
{
